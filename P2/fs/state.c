...
void lock(int i){
    if(pthread_rwlock_wrlock(&inode_table[i].rwlock)!=0){
        printf("Erro ao dar lockwr no %d!\n", i);
        return exit(EXIT_FAILURE);
    }
}

void lockrd(int i){
    if(pthread_rwlock_rdlock(&inode_table[i].rwlock)!=0){
        printf("Erro ao dar lockrd no %d!\n", i);
        return exit(EXIT_FAILURE);
    }
}

void unlock(int i){
    if(pthread_rwlock_unlock(&inode_table[i].rwlock)!=0){
        printf("Erro ao dar unlock no %d!\n", i);
        return exit(EXIT_FAILURE);
    }
}
